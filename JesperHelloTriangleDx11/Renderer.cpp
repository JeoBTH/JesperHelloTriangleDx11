...
void Renderer::beginFrame()
{
	...
	float red = (float) (rand() % 255 / 2555.0f); // Random color to see how it updates
	float clearColor[] = { red, .25f, .25f, 1 }; // RGBA
	this->m_deviceContext->ClearRenderTargetView(m_renderTargetView, clearColor);
}
...
void Renderer::createRenderTarget()
{
	...
	this->m_SwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&backBuffer); // how you get the buffer from the swap chain. index of buffer, what type, to where.
	this->m_device->CreateRenderTargetView(backBuffer, nullptr, &m_renderTargetView);
